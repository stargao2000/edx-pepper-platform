...
(function() {
    ...
    StaffGradingBackend.prototype.post = function(cmd, data, callback) {
      ...
      if (this.mock_backend) {
        ...
        return $.post(this.ajax_url + cmd, data, callback).error(function() {
          return callback({
            success: false,
            error: "Error occured while performing javascript AJAX post."
          });
        });
      }
    };
  ...
  this.StaffGrading = (function() {
    ...
    StaffGrading.prototype.submit_and_get_next = function() {
      ...
      this.gentle_alert("Grades saved.  Fetching the next submission to grade.");
      return this.backend.post('save_grade', data, this.ajax_callback);
    };
    ...
    StaffGrading.prototype.render_view = function() {
    ...
    StaffGrading.prototype.render_problem = function() {
      var meta_list, problem_list_link, show_action_button, show_submit_button,
        _this = this;
      show_submit_button = true;
      show_action_button = true;
      problem_list_link = $('<a>').attr('href', 'javascript:void(0);').append("< Back to problem list").click(function() {
        return _this.get_problem_list();
      });
      this.breadcrumbs.append(problem_list_link);
      if (this.state === state_error) {
        this.set_button_text('Try loading again');
        show_action_button = true;
      } else if (this.state === state_grading) {
        this.ml_error_info_container.html(this.ml_error_info);
        meta_list = $("<div>");
        meta_list.append("<div class='meta-info'>" + this.num_pending + " available | </div>");
        meta_list.append("<div class='meta-info'>" + this.num_graded + " graded | </div>");
        meta_list.append("<div class='meta-info'>" + (Math.max(this.min_for_ml - this.num_graded, 0)) + " more needed to start ML </div><br/>");
        ...
        this.set_button_text('Submit');
        show_action_button = false;
      } else if (this.state === state_no_data) {
        this.message_container.html(this.message);
        this.set_button_text('Re-check for submissions');
      } else {
        this.error('System got into invalid state ' + this.state);
      }
    ...
    StaffGrading.prototype.submit = function(event) {
      ...
      if (this.state === state_error) {
        ...
        return this.error('System got into invalid state for submission: ' + this.state);
      }
    };
